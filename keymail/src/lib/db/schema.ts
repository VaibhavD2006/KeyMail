...
export const milestoneRelations = relations(milestones, ({ one }) => ({
  ...
  client: one(clients, {
    fields: [milestones.clientId],
    references: [clients.id],
  }),
  emailTemplate: one(emailTemplates, {
    fields: [milestones.emailTemplateId],
    references: [emailTemplates.id],
  }),
}));
...
export const showingRelations = relations(showings, ({ one, many }) => ({
  user: one(users, {
...
export const showingFeedbackRelations = relations(showingFeedback, ({ one }) => ({
  showing: one(showings, {
    fields: [showingFeedback.showingId],
    references: [showings.id],
  }),
  client: one(clients, {
    fields: [showingFeedback.clientId],
    references: [clients.id],
  }),
}));

export const propertyMatchRelations = relations(propertyMatches, ({ one }) => ({
  user: one(users, {
    fields: [propertyMatches.userId],
    references: [users.id],
  }),
  client: one(clients, {
    fields: [propertyMatches.clientId],
    references: [clients.id],
  }),
  listing: one(listings, {
    fields: [propertyMatches.listingId],
    references: [listings.id],
  }),
  sentEmail: one(emailHistory, {
    fields: [propertyMatches.sentEmailId],
