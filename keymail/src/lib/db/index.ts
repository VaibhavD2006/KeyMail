...
console.log(`Initializing database connection using ${connectionString.replace(/:.+@/, ":****@")}`);
...
try {
  ...
  client = createMockClient();
  db = drizzle(client, { schema });
  console.error("CRITICAL: Database connection unavailable. Using localStorage fallback.");
}
